  ...
  checkSchema(
    {
      ...
      bio: {
        ...
        isLength: {
          ...
          errorMessage: USERS_MESSAGES.BIO_LENGTH_MUST_BE_LESS_THAN_200 //messages.ts thêm LOCATION_LENGTH_MUST_//trim phát đặt cuối, nếu k thì nó sẽ lỗi validatior
        }
      },
      //giống bio
      location: {
      ...
      followed_user_id: {
        custom: {
          options: async (value: string, { req }) => {
            ...
            //vào database tìm user đó xem có không ?
            const followed_user = await databaService.users.findOne({
              _id: new ObjectId(value)
            })
